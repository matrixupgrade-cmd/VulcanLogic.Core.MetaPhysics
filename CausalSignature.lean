...
/-!
  ...
  If both paths were genuinely at tolerance ε_inv, their
  CumulativeDeviation sequences would agree (both tracking
  the same source under ApproximateClarity). Divergence
  in CUSUM is therefore a certificate of path degradation,
  not source ambiguity.

  The asymmetry is useful: if one observer's CUSUM tracks
  the expected source pattern and the other's does not,
  the degraded path is identified, not merely the fact
  that degradation occurred.

  Formally: disagreement count * ε_inv > T for at least
  one of obs₁, obs₂ — i.e. ApproximateClarity fails at
  the certified tolerance for that observer.

  This is the diagnostic direction of the bridge:
  internal certificate + boundary mismatch → path interference.
-/
theorem false_signal_mismatch
    ...
    (h_cl₁  : ApproximateClarity Node bg sig src obs₁ ε_inv T)
    (h_cl₂  : ApproximateClarity Node bg sig src obs₂ ε_inv T)
    -- their CUSUM sequences disagree
    (h_diff : CumulativeDeviation Node bg sig obs₁ T ≠
              CumulativeDeviation Node bg sig obs₂ T) :
    -- at least one path lacks exact clarity; the signal encoding is inconsistent
...
/-!
  ...
  When one observer is known good (its CUSUM tracks src),
  the other's divergence is attributable to its path,
  not to source ambiguity.

  Applied use: obs_ref is a low-ε probe at a known good vantage
  point. obs_suspect is the probe under test. CUSUM divergence
  between them, given shared source, localizes the interference
  to the path src → obs_suspect.
-/
