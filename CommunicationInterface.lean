...
-- ==================================================
/-
  This file is a seed for future work. The structure is not
  arbitrary — several choices were forced by the mathematics
  itself, and future extensions should understand why before
  changing them.

  1. WHY `h_dist_from_sig` IS AN EXPLICIT HYPOTHESIS, NOT A LEMMA

     `interface_distortion` is ℝ-valued: it sums raw asymmetry
     values over junction nodes.
     `canonical_signature` is Fin-valued: it buckets those values
     into discrete bins via `bucket : ℝ → Fin entropy`.
     There is no definitional link between them in Lean — the
     bucketing function is opaque. No amount of unfolding closes
     the gap between a real-valued sum and a finite-valued map.

     This is not a Lean limitation to be engineered around. It is
     the honest statement of where measurement theory enters: any
     claim that "same signature implies same distortion" is a
     *physical* assumption about how the bucketing is chosen, not a
     mathematical consequence of the definitions. Carrying it as an
     explicit hypothesis makes that dependency visible at every call
     site, which is exactly what you want in a seed file.

  2. WHY `basin_trajectory` IS `Finset.image`, NOT A `List` OR BARE `Finset`

     Order does not matter for the trajectory — a basin visits a
     set of signatures, not a sequence. `List` would introduce
     spurious ordering and duplicate-counting.
     A bare `Finset` without the image construction would lose the
     functional relationship between signals and signatures — you
     would not be able to prove trajectory bounds from basin entropy.
     `Finset.image (fun s => canonical_signature ...) b.signals` is
     exactly what the mathematics says: the image of the basin's
     signal set under the signature map. The `card_image_le` bound
     then falls out immediately from the definition.

  3. WHY `configsInBasin` GOES THROUGH `Finset → List`

     `observable_complexity` is defined over a `ConfigSpace = List`,
     because config application is naturally sequential and lists
     allow duplicate probes (two signals may map to the same config;
     that is intentional — it is what `toFinset.card` collapses).
     The `Finset → List` conversion via `toList` is fiddly but
     correct: `List.toFinset_toList` recovers the Finset roundtrip,
     and `card_image_le` handles the entropy bound.
     Do not try to replace `ConfigSpace` with `Finset` without
     revisiting every proof that pattern-matches on list structure.

  4. WHY THE INJECTIVE MAP IN `distortions_le_basin_sig_image`
     IS CONSTRUCTED EXPLICITLY RATHER THAN VIA `Finset.card_image_le`

     The distortion image and the signature image live in different
     types (ℝ and `LocalSignature`). There is no direct image map
     from one Finset to the other — you have to go through the
     witness chain: distortion value → config → signal → signature.
     The explicit injection φ makes that chain a first-class Lean
     term, which is what `Fintype.card_le_of_injective` requires.
     This is the most complex proof in the file and the one most
     likely to need updating if the type structure changes.

  5. WHY `BoundaryJunction` CARRIES UNUSED SEPARATION FIELDS

     `h_disjoint`, `h_sep_A`, `h_sep_B` are not referenced by any
     current proof. They are carried because patchwork probes and
     bridge optimization (future work) need to reason about which
     paths cross the boundary — and retrofitting those fields later
     would require updating every call site. Lean does not complain
     because no current proof pattern-matches on them.
     See the "Future directions" block in the adaptive basin section.

  6. THE TAUTOLOGY IN `adaptive_basin_reconstruction`

     The proof is `le_or_lt` — a tautology. This is intentional.
     The theorem's content is not the proof but the *decomposition*
     it licenses: within each constructive regime a local optimum
     exists and is bounded; only destructive interference is a
     genuine source of complexity growth. The interesting question
     (when does destructive interference actually occur?) requires
     a concrete `g` and basin pair and is deferred to future files.
-/
