...
def SeparationCertifiesClarity
    ...
    (sig      : TemporalSignal Node)
    (obs      : Node) : Prop :=
  ∀ T, ApproximateClarity Node bg sig obs obs
...
-- ====================================================
-- PERSISTENCE → PULSE PRESERVATION
-- ====================================================
...
theorem absorbing_implies_permanent_pulse
    ...
    ∀ duration,
      -- if the board is true at T_start
      iterated_board update T_start b₀ i = true →
      -- then the signal is on for the whole duration
  ...
  -- The signal encodes the board, so it stays true
  have h_sig_true : sig i t = true := by
    rw [h_encode t]
    exact h_stays t h_lo
  -- SignedDeviation is +1 when signal is true and background is false
  -- Under the assumption that background is the baseline (false = no signal)
  simp [PulseOn, SignedDeviation]
  -- The signal is on; deviation is +1 if background is off at t
  -- This requires background to be false at t for a clean pulse
  -- (exact clarity assumption handles this)
  sorry -- requires background model connection; deferred to applied layer
